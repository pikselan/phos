                                                    ...
                                                    <?php foreach ($brands as $brand): ?>
                                                        <div class="col-sm-12 col-sm-3">
                                                            <div class="custom-control custom-radio custom-control-validate-input">
                                                                <input type="radio" class="custom-control-input" id="form_radio_<?= $brand->id; ?>" name="brand_id"
                                                                       value="<?= $brand->id; ?>" <?= $product->brand_id == $brand->id ? 'checked' : ''; ?> <?= $productSettings->is_brand_optional != 1 ? 'required' : ''; ?>>
                                                                <label class="custom-control-label" for="form_radio_<?= $brand->id; ?>"><?= esc(getBrandName($brand->name_data, selectedLangId())); ?></label>
                                                            </div>
                                                        </div>
                                                    <?php endforeach; ?>
                                                </div>
                        ...
                        <div class="form-box">
                            ...
                            <div class="form-box-body">
                                <div class="row">
                                    ...
                                    $states = !empty($product->country_id) ? getStatesByCountry($product->country_id) : array();
                                        ...
                                        <select id="select_countries" name="country_id" class="select2 form-control" onchange="getStates(this.value);">
                                            ...
                                            <?php if (!empty($countries)):
                                                foreach ($countries as $item):
                                                    if ($item->status == 1):?>
                                                        <option value="<?= $item->id; ?>" <?= !empty($product->country_id) && $item->id == $product->country_id ? 'selected' : ''; ?>><?= esc($item->name); ?></option>
                                                    <?php endif;
                                                endforeach;
                                            endif; ?>
                                        </select>
                                    </div>
                                    <div id="get_states_container" class="col-md-12 col-lg-2 m-b-15 <?= !empty($product->country_id) ? '' : 'display-none'; ?>">
