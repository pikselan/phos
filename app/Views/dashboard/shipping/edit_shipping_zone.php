                                    ...
                                    if (!empty($locations)):
                                        ...
                                        foreach ($locations as $location):
                                            if (!empty($location->country_name) && !empty($location->state_name)):
                                                ...
                                                <div id="lc-state-<?= $location->state_id; ?>" class="region"><?= esc(getContinentNameByKey($location->continent_code)) . '/' . esc($location->country_name) . '/' . esc($location->state_name); ?><a href="javascript:void(0)" onclick="deleteShippingLocation('<?= $location->id; ?>');"><i class="fa fa-times"></i></a><input type="hidden" value="<?= $location->state_id; ?>" name="state[]"></div>
                                            <?php elseif (!empty($location->country_name) && empty($location->state_name)):
                                                array_push($arrayRegions, 'country-' . $location->country_id); ?>
                                                <div id="lc-country-<?= $location->country_id; ?>" class="region"><?= esc(getContinentNameByKey($location->continent_code)) . '/' . esc($location->country_name); ?><a href="javascript:void(0)"><i class="fa fa-times" onclick="deleteShippingLocation('<?= $location->id; ?>');"></i></a><input type="hidden" value="<?= $location->country_id; ?>" name="country[]"></div>
                                            <?php else:
