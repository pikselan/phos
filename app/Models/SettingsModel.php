...
class SettingsModel extends BaseModel
{
    ...
    //get global taxes
    public function getGlobalTaxes()
    {
        $sData = unserializeData($this->paymentSettings->global_taxes_data);
    ...
    //add global tax
    public function addGlobalTax()
    {
        $paymentSettings = $this->getPaymentSettings();
        if (!empty($paymentSettings)) {
            $globalTaxArray = array();
            $sData = unserializeData($paymentSettings->global_taxes_data);
            if (!empty($sData) && is_array($sData)) {
                $globalTaxArray = $sData;
            }
            //tax name
            $taxNameArray = array();
            ...
            $inputCountries = inputPost('country');
            $taxId = uniqid();
            $item = [
                'id' => $taxId,
                'taxNameArray' => $taxNameArray,
                'taxRate' => inputPost('tax_rate'),
                'status' => !empty(inputPost('status')) ? 1 : 0,
                'isAllCountries' => inputPost('all_countries') == 1 ? 1 : 0,
                'countryIds' => inputPost('country')
            ];
            if ($item['isAllCountries']) {
                $item['countryIds'] = array();
            }
            array_push($globalTaxArray, $item);
            return $this->builderPaymentSettings->where('id', 1)->update(['global_taxes_data' => serialize($globalTaxArray)]);
        }
        return false;
    }

    //add global tax
    public function editGlobalTax()
    {
        $taxId = inputPost('id');
        if ($this->deleteGlobalTax($taxId)) {
            return $this->addGlobalTax();
        }
        return false;
    }

    //delete global tax
    public function deleteGlobalTax($taxId)
    {
        if (!empty($taxId)) {
            $globalTaxesArray = array();
            $newGlobalTaxesArray = array();
            $sData = unserializeData($this->paymentSettings->global_taxes_data);
            if (!empty($sData) && is_array($sData)) {
                $globalTaxesArray = $sData;
            }
            if (!empty($globalTaxesArray)) {
                foreach ($globalTaxesArray as $tax) {
                    if ($tax['id'] != $taxId) {
                        array_push($newGlobalTaxesArray, $tax);
                    }
                }
            }
            return $this->builderPaymentSettings->where('id', 1)->update(['global_taxes_data' => serialize($newGlobalTaxesArray)]);
        }
        return false;
    }
